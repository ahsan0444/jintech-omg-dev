...
- **MCP first.** In `lib/`, `public/javascripts/`, `t/` of OMG repos, use code-review-graph MCP tools (semantic_search → query_graph → traverse_graph). MCP tools are deferred — load schemas via ToolSearch (`select:mcp__...`) before first call. Grep there is hook-blocked; do not retry it.
